...
const schema=z.object({
  name:z.string().min(1), cardNumber:z.string().optional().nullable(), setName:z.string().optional().nullable(), year:z.number().int().optional().nullable(), variant:z.string().optional().nullable(), category:z.string().optional().nullable(),
...
export async function POST(req:Request){
  ...
  const existing=await db.select({id:cardPrintings.id}).from(cardPrintings).where(and(eq(cardPrintings.name,p.name),p.cardNumber?eq(cardPrintings.cardNumber,p.cardNumber):isNull(cardPrintings.cardNumber),p.setName?eq(cardPrintings.setName,p.setName):isNull(cardPrintings.setName),variant?eq(cardPrintings.variant,variant):isNull(cardPrintings.variant))).limit(1);
  let printingId=existing[0]?.id;
  if(!printingId){const [printing]=await db.insert(cardPrintings).values({name:p.name,cardNumber:p.cardNumber,setName:p.setName,year:p.year,variant,category:p.category}).returning({id:cardPrintings.id}); printingId=printing.id;}
