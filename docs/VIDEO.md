...
8. Mark the recoverable processing job complete, or retain its failure/attempt details.
...
The worker rejects oversized/overlong inputs, enforces UUID and storage-path ownership, uses bounded FFmpeg timeouts, and requires the same `VIDEO_WORKER_SECRET` as the web service. Its `/health` endpoint reports actual FFmpeg/OpenCV availability and returns HTTP 503 when processing dependencies are missing.
