...
def fail_job(client: Any, request: ProcessRequest, message: str) -> None:
    client.table("media_assets").update({"processing_status": "failed"}).eq("id", str(request.media_asset_id)).execute()
    update_job(client, str(request.job_id), status="failed", error=message[:2000])
...
@app.post("/process")
def process(request: ProcessRequest, x_worker_secret: str | None = Header(default=None)):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            old_contacts = client.table("media_assets").select("id,storage_path").eq("physical_card_id", card_id).eq("kind", "contact_sheet").eq("capture_type", contact_type).execute().data or []
            if old_contacts:
                client.storage.from_(BUCKET).remove([row["storage_path"] for row in old_contacts])
                client.table("media_assets").delete().eq("physical_card_id", card_id).eq("kind", "contact_sheet").eq("capture_type", contact_type).execute()
